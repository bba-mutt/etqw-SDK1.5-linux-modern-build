...
// Capture esp directly. At all three call sites (init/save/restore), esp has the same value
// (function prologue complete, no pending sub/push). B's save range ends at esp_B_init which
// is below CE's own locals, so CE's frame is never clobbered before A's restore fixes it.
...
// On i386 PIC, the compiler caches the GOT base in [ebp-N] and reloads %ebx from it
// after calls like getcontext/setcontext. If the stack slot is corrupted by a coroutine
// restore memcpy, the reloaded %ebx points into .text and any write through it faults.
// Using noinline helpers forces a fresh GOT-base computation in each helper's own prologue,
// so context_jump is always accessed with a valid GOT pointer regardless of CE's frame state.
__attribute__((noinline)) static bool context_jump_is_set( void ) {
	return context_jump;
}

__attribute__((noinline)) static bool context_jump_test_and_clear( void ) {
	if ( !context_jump ) return false;
	context_jump = false;
	return true;
}

__attribute__((noinline)) static void context_jump_set( void ) {
	context_jump = true;
}

// The restore path in Coroutine_Enter runs with a corrupted [ebp-0x1c] (the compiler's
// cached GOT base), because the coroutine that ran between save and restore overwrote that
// stack slot. Every access to s_current or any PLT call in CE would reload %ebx from that
// corrupted slot and fault. This helper computes its own fresh GOT base, reads s_current,
// does the restore memcpy, and frees the saved stack — all without touching CE's frame.
// The STACK_GET (capturing CE's ESP) is done in CE before the call so the memcpy target
// address is correct; by the time this helper returns, CE's frame has been restored and
// the epilogue's pop/ret instructions read back the correct saved registers and return address.
__attribute__((noinline)) void coroutine_restore_path( char* stack_ptr ) {
	sdDLLThread* cur = sdDLLThread::s_current;
	if ( cur->IsStackSaved() ) {
		assert( cur->storedStackPointer - cur->storedStackSize == stack_ptr );
		memcpy( stack_ptr, cur->localStack, cur->storedStackSize );
		cur->FreeStack();
	}
}
#endif

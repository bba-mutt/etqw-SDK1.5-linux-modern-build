# -*- mode: python -*-
import sys, os, time, re, pickle, pdb, zipfile, io, subprocess
import SCons
...
class idBuffering:
	...
	def buffered_spawn( self, sh, escape, cmd, args, env ):
		...
		stderr_text = ''
		try:
			result = subprocess.run( command_string, shell=True, executable=sh, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env )
...
def ExtractSource( file ):
	...
	for i in files:
		...
		s = re.sub( '\\\\', '/', s )
		s = re.sub( r'^\./', '', s )
		if ( s[-4:].lower() == '.cpp' or s[-2:].lower() == '.c' ):
			l.append( s )
	return l
